 ...
 *
 * Safe to re-run. Every step checks for what it is about to create and skips it,
 * so this tops up a partial database rather than duplicating one. `pnpm seed`
 * twice in a row is a no-op the second time.
 *
 * Never run against production; guarded below. `pnpm seed:reset` removes
 * exactly what this creates and nothing else.
 */
...
import { emptyManifest, record, saveManifest, type Manifest } from './manifest'
...
// Only the Payload config has to wait for dotenv. Everything above is pure.
