...
export const POST = withRateLimit(
  ...
  /**
   * Twenty a minute, counted in Postgres rather than in this process.
   *
   * This route writes a booking and sends at least one email, so it is in the
   * "costs something to be wrong about" category that the auth routes are in -
   * and a per-instance tally is not a limit when several instances are warm.
   * Its own budget rather than the auth one so a customer booking dinner cannot
   * spend the allowance that lets them sign in. See lib/rate-limit.
   */
